...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    // the global scope are not tracked by the resolver since they are more dynamic in Lox. When
    ...
    public Void visitSuperExpr(Expr.Super expr) {
        ...
        // The resolution stores the number of hops along the environment chain that the
        // interpreter needs to walk to find the environment where the superclass is stored.
    ...
    private void define(Token name) {
        if (scopes.isEmpty()) return;
        scopes.peek().put(name.lexeme(), true);  // marked as fully initialized
    }

    // We start at the innermost scope and work outwards, looking in each map for a matching name.
    // If we find the variable, we resolve it, passing in the number of scopes between the current
    // innermost scope and the scope where the variable was found. So if the variable was found in
    // the current scope, we pass in 0. If it's in the immediately enclosing scope, 1.
